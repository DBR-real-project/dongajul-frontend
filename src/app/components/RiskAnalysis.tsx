          ...
          <div className="grid grid-cols-3 gap-6">
            {[
              ...
              <div key={label} className={`${darkMode ? 'bg-gray-900/30' : 'bg-gray-50/50'} p-4 rounded-xl text-center`}>
                ...
                <div className={`text-sm font-medium ${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-2`}>{label}</div>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.tags.map((tag) => (
                        <span key={tag} className={`px-2.5 py-1 ${darkMode ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-700'} text-xs font-medium rounded-lg`}>{tag}</span>
                      ))}
                    </div>
                  </div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.tags.map((tag) => (
                        <span key={tag} className={`px-2.5 py-1 ${darkMode ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-700'} text-xs font-medium rounded-lg`}>{tag}</span>
                      ))}
                    </div>
                  </div>
